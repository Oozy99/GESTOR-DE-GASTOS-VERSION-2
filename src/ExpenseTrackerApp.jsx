...
const ExpenseTrackerApp = () => {
  ...
  const handleLogin = async () => {
    ...
    if (!loginForm.username.trim()) {
      setAuthError('Ingresa tu nombre'); return;
    }
    setAuthLoading(true);
    const user = authenticate(loginForm.username);
    setAuthLoading(false);
    if (!user) { setAuthError('Ese nombre no tiene acceso'); return; }
  ...
  const loadAllData = async () => {
    ...
    const [{ data: fixed }, { data: general }, { data: cats }, { data: sals }] = await Promise.all([
      db.getGastosFijos(uid),
      db.getGastosGenerales(uid),
      db.getCategorias(uid),
      db.getSueldos(uid),
    ]);
  ...
  const addFixedExpense = async () => {
    ...
    if (editingFixed) {
      const { data } = await db.updateGastoFijo(editingFixed.id, payload);
      ...
      const { data } = await db.insertGastoFijo(payload);
  ...
  const renewFixedExpense = async (expense, fechaManual = null) => {
    ...
    const { data } = await db.updateGastoFijo(expense.id, {
    ...
    if (data) {
      ...
      const { data: existentesFijo } = await db.findGastosGeneralesByDescripcion(
        currentUser.id, descripcionFijo
      );
      ...
      if (sueldoDestino) {
        ...
        if (fueraDePeriodo.length > 0) {
          ...
          await db.deleteGastosGeneralesByIds(ids);
        ...
        if (enPeriodoCorrecto.length > 1) {
          ...
          await db.deleteGastosGeneralesByIds(idsEliminar);
          ...
          const { data: insertado } = await db.insertGastoGeneral(nuevo);
          if (insertado) {
  ...
  const toggleFixedExpense = async (expense) => {
    ...
    const { data } = await db.updateGastoFijo(expense.id, payload);
    if (data) setFixedExpenses(prev => prev.map(e => e.id === data.id ? mapFixed(data) : e));
  };
  ...
  const addGeneralExpense = async () => {
    ...
    if (editingGeneral) {
      const { data } = await db.updateGastoGeneral(editingGeneral.id, payload);
      if (data) setGeneralExpenses(prev => prev.map(e => e.id === data.id ? mapGeneral(data) : e)
        .sort((a,b) => parseColDate(a.fecha) - parseColDate(b.fecha)));
      setEditingGeneral(null);
    } else {
      const { data } = await db.insertGastoGeneral(payload);
  ...
  const deleteGeneralExpense = async (id) => {
    await db.deleteGastoGeneral(id);
  ...
  const addCategory = async () => {
    ...
    if (editingCategory) {
      const { data } = await db.updateCategoria(editingCategory.id, { nombre: newCategory });
      ...
      const { data } = await db.insertCategoria({ usuario_id: currentUser.id, nombre: newCategory });
  ...
  const deleteCategory = async (cat) => {
    ...
    await db.deleteCategoria(cat.id);
  ...
  const addSalary = async () => {
    ...
    // 1. Guardar el sueldo
    const { data } = await db.insertSueldo({
      usuario_id: currentUser.id,
    ...
    if (data) {
      ...
      const { data: existentes } = await db.findFijosPrefixInRange(
        currentUser.id, salary.fechaPago, endStr
      );
      ...
      if (nuevosGastos.length > 0) {
        const { data: insertados } = await db.insertGastosGeneralesBatch(nuevosGastos);
        if (insertados) {
          setGeneralExpenses(prev =>
  ...
  const deleteSalary = async (id) => {
    ...
    await db.deleteSueldo(id);
    setSalaries(prev => prev.filter(s => s.id !== id));
  };

  // ─── EXPORTAR / RESPALDAR DATOS ─────────────────────────────────────────────

  // Descarga TODA la información registrada (fijos, generales, sueldos y
  // categorías) en un único archivo CSV, con encabezados y agrupada por
  // sección para saber a qué tabla pertenece cada registro.
  const exportToCSV = () => {
    const esc = (val) => {
    ...
    rows.push(['Servicio', 'Categoría', 'Precio', 'Frecuencia', 'Fecha Renovación', 'Próxima Renovación', 'Activo', 'Último Refresco']);
    fixedExpenses.forEach(e => rows.push([
      e.servicio, e.categoria, e.precio, e.frecuencia,
      e.fechaRenovacion, e.proximaRenovacion, e.activo ? 'Sí' : 'No', e.ultimoRefresco || '',
    ]));
    rows.push([]);

    rows.push(['GASTOS GENERALES']);
    rows.push(['Descripción', 'Precio', 'Mes', 'Fecha', 'Año']);
    generalExpenses.forEach(e => rows.push([e.descripcion, e.precio, e.mes, e.fecha, e.año]));
    rows.push([]);

    rows.push(['SUELDOS']);
    rows.push(['Monto', 'Frecuencia', 'Fecha de Pago']);
    salaries.forEach(s => rows.push([s.monto, s.frecuencia, s.fechaPago]));
    rows.push([]);

    rows.push(['CATEGORÍAS']);
    rows.push(['Nombre']);
    categories.forEach(c => rows.push([c.nombre]));
    ...
    link.href = url;
    link.download = `gastos_${currentUser.username}_${todayCol()}.csv`;
  ...
  // Respaldo completo en JSON (útil para mover la información a otro dispositivo)
  const exportBackupJSON = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `respaldo_gastos_${todayCol()}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  ...
  // Restaurar un respaldo JSON previamente descargado
  const importBackupJSON = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!window.confirm('Esto reemplazará los datos actuales guardados en este dispositivo por los del respaldo. ¿Continuar?')) return;
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-500 text-sm mt-2">Ingresa tu nombre para acceder</p>
          </div>
          {authError && <div className="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm">{authError}</div>}
          <div className="space-y-4">
            <input type="text" placeholder="Nombre" value={loginForm.username}
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex gap-2 flex-wrap items-center">
            <button onClick={exportToCSV}
              title="Descargar toda la información en un archivo CSV"
            ...
            <button onClick={exportBackupJSON}
              title="Descargar un respaldo completo en JSON"
              className="bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-400 font-semibold flex items-center gap-2">
              <FileJson className="w-4 h-4"/>Respaldo
            </button>
            <label
              title="Restaurar un respaldo JSON previamente descargado"
              className="bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-400 font-semibold flex items-center gap-2 cursor-pointer">
              <Upload className="w-4 h-4"/>Restaurar
              <input type="file" accept="application/json" className="hidden"
                onChange={e => { importBackupJSON(e.target.files?.[0]); e.target.value = ''; }} />
            </label>
